...
ini_set( "display_errors", 0); 
  ...
  <div class="form-group">
    ...
    <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter Restaurant ID " name="username">
...
$ext = pathinfo($filename, PATHINFO_EXTENSION);
            ...
            $sql = "INSERT INTO addres(username,resname,resadd,rescity,resdes,name,image)
                                VALUES('$username','$resname','$resadd','$rescity','$resdes','$filename','$tmpname')";
      
                 if(empty($username) ||  empty($resname) || empty($resadd) || empty($rescity) || empty($resdes)|| empty($filename) || !in_array($ext, $array)){
                echo"<script>swal({
